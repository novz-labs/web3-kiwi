  ...
  (
    ...
    const renderUIComponent = () => {
      ...
      switch (type) {
        case "menu":
          return (
            <div className="border border-primary/20 rounded p-2 mb-2 bg-background/50">
              <div className="text-primary text-xs mb-2">MENU (Use ↑↓ arrows, ENTER to select)</div>
              {props.items?.map((item: string, index: number) => (
                <div
                  key={index}
                  className={cn(
                    "px-2 py-1 font-mono text-sm",
                    index === opentuiState[0].menuSelection ? "bg-primary text-background" : "text-primary",
                  )}
                >
        ...
        case "form":
          return (
            <div className="border border-primary/20 rounded p-2 mb-2 bg-background/50">
              <div className="text-primary text-xs mb-2">FORM (TAB to navigate, ENTER to submit, ESC to cancel)</div>
              {props.fields?.map((field: string, index: number) => (
                <div key={index} className="mb-2">
                  <label className="text-primary text-sm block mb-1">{field}:</label>
                  <input
                    ref={(el) => {
                    ...
                    type="text"
                    className="w-full bg-transparent border border-primary/20 rounded px-2 py-1 text-foreground font-mono text-sm focus:border-primary outline-none"
    ...
    const getVariantStyles = () => {
      switch (variant) {
        ...
        default:
          return "p-4 text-sm shadow-2xl shadow-primary/10"
      }
    }
        ...
        >
          {variant !== "minimal" && (
            <div className="flex items-center justify-between mb-2 pb-2 border-b border-primary/20">
              ...
              <div className="text-primary/60 text-xs">Ctrl+L to clear</div>
            </div>
          )}
          ...
          >
            ...
            <div className="flex items-center text-foreground mt-1 relative">
              ...
              <div className="flex-1 relative">
                ...
                />
                <div
                  className="absolute top-0 w-2 h-5 bg-primary pointer-events-none animate-terminal-blink"
                  style={{
                    left: `${cursorPosition * 0.6}em`,
                  }}
                />
              </div>
              {isProcessing && <span className="ml-2 text-primary animate-pulse">⚡</span>}
            </div>
          </div>
        </div>
